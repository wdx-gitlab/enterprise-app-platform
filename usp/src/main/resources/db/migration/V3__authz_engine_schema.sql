-- Authz Engine 权限引擎完整表结构基线。
-- 当前仓库把多个模块的 SQL 暂时放在同一 migration 目录中，但该文件仍按 Authz 模块独立维护。
-- 当前版本号是共享 Flyway 目录中的全局 V3 段。
-- 后续如果按模块拆分 Flyway location，本文件应归入 Authz 独立迁移目录。
